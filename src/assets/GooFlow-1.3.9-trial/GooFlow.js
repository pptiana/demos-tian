...
//初始化区域图的对象
  this.$id=bgDiv.attr("id");
	...
	this.$title="资源编排";//流程图的名称
	this.$nodeRemark={};//每一种结点或按钮及top顶部的说明文字,JSON格式,key为类名,value为用户自定义文字说明
  ...
  var lineToType = '';
	if(property.haveHead){
      ...
      tmp+="<label title='"+(property.initLabelText||"资源编排")+"' "
        +(GooFlow.prototype.color.main? "style='background:"+GooFlow.prototype.color.main+"'" : "")+">"+(property.initLabelText||"资源编排")+"</label>";
    }
    //tian  顶部图标加ID  便于加title
		for(var x=0;x<property.headBtns.length;++x){
			tmp+="<a href='javascript:void(0)' class='GooFlow_head_btn' id='btn_"+property.headBtns[x]+"'><i class='ico_"+property.headBtns[x]+"'></i></a>"
		}
		tmp+="</div>";
		this.$head=$(tmp);
        ...
        case "ico_print fa fa-print"	:This.onPrintClick();break;
        case "ico_exportImg fa fa-external-link":This.onExportImgClick();break;
			}
		});
	}
	var toolWidth=0;
...
GooFlow.prototype={
  ...
  setHeadToolsRemarks:function(remark){
	...
	initWorkForNode:function(){
      ...
      This.$draw.appendChild(line);
      //tian 记录初始节点的type值
      // lineFromType = $(this).find('i').attr('class').split(' ')[0].split('ico_')[1];
		});
		//绑定连线时确定结束点
		this.$workArea.delegate(".GooFlow_item","mouseup",{inthis:this},function(e){
      //tian 记录结束节点的type值
      // lineToType = $(this).find('i').attr('class').split(' ')[0].split('ico_')[1];
      // 判断初始节点和结束节点的type值 确定是否可以连线
      // if (lineFromType && lineFromType === 'moon' && lineToType && lineToType === 'sun' ) {
      //   console.log('此方向不允许连线');
      //   return  false;
      // }
			var This=e.data.inthis;
			if(This.$nowType!="direct"&&!This.$mpTo.data("p"))	return;
